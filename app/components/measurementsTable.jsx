    ...
    return (
      <Table striped bordered hover className="measurementsTable">
        <thead>
          <tr>
            <th></th>
            {this.props.bicycles.map((bicycle, index) => (<th key={bicycle.name} className={`bicycle-${index + 1}`}>{bicycle.brand} {bicycle.model} {bicycle.size} <Button bsSize="xsmall" onClick={() => this.unselect(bicycle.name)}>X</Button></th>))}
          </tr>
        </thead>
        <tbody>
